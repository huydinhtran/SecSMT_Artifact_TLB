...
print_stats(void)
{
    ...
    printf("\n=============== l2fwd statistics ================\n"
		   ...
		   "\nTotal packets / microseconds: %.3f",
           total_packets_tx*3000000,
           total_packets_rx*3000000,
           elapsed_microseconds,
		   ((total_packets_rx*3000000) / (double)elapsed_microseconds));
    printf("\n====================================================\n");
	total_packets_tx = 0;
	total_packets_rx = 0;
...
l2fwd_main_loop(pcap_t *handle)
{
	...
	for (i = 0; i < 100004; i++) {
		...
		if (m == NULL) {
			...
			// if (port_statistics[0].rx % 1000000 == 0) {
				// print_stats(); 
			// }
